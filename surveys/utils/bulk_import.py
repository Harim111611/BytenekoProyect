        ...
        with connection.cursor() as cursor:
            ...
            for idx, row in enumerate(rows):
                ...
                for column_name, value in row.items():
                    # Only process columns present in questions_map and with non-empty value
                    if column_name not in questions_map or value is None or (isinstance(value, str) and not value.strip()):
                        continue
                    ...
                    if dtype == 'single':
                        ...
                        options = question_data.get('options', {})
                        # Split comma-separated values and create a response for each
                        for opt in str(value).split(','):
                            opt_clean = opt.strip()
                            if not opt_clean:
                                continue
                            option_id = options.get(opt_clean)
                            if not option_id:
                                continue
                            answer_buffer.write(f"{survey_response_id}\t{question.id}\t{option_id}\t\\N\t\\N\n")
                            answers_count += 1
